...
def jacobian_opt_compiled(
    ...
    w_enc_active = sae_mlpout_W_enc_T_param[out_idx]
    ...
    w_dec_active = sae_mlpin_W_dec_T_param[:, in_idx].permute(1, 2, 0, 3)
    # Expected shape: (batch, seq, d_model, k1)

    # 2. Calculate j_after_inlined
    j_after_inlined = torch.einsum("bskd,dm,bsm,mi->bski",
                                   w_enc_active,
                                   mlp_W_out_weight,
                                   mlp_grads,
                                   mlp_W_in_weight)
    ...
    id_term = torch.einsum("bsKi,i,bsir->bsKr",
                           j_after_inlined, ln_gamma, w_dec_active)
    id_component = id_scale_scalar_squeezed.unsqueeze(-1).unsqueeze(-1) * id_term

    term1_off_ones = torch.einsum("bsKi,i->bsK", j_after_inlined, ln_gamma)
    ...
    off_y_left = torch.einsum("bsKi,bsi->bsK", j_after_inlined, gamma_ln_pre_y)
...
def jacobian_opt(out_idx: Int[Tensor, "batch seq k2"],
    ...
    sae_mlpout_W_enc_T_val = sae_mlpout.W_enc.T
    sae_mlpin_W_dec_T_val = sae_mlpin.W_dec.T
    ...
    return jacobian_opt_compiled(
        out_idx, in_idx,
        sae_mlpout_W_enc_T_val,  # Pass W_enc.T
        sae_mlpin_W_dec_T_val,   # Pass W_dec.T
        mlp_W_out_weight, mlp_W_in_weight,
        mlp_grads,
...
sparse_block_jacobian_opt = jacobian_opt(z_2_idx, z_idx, sae_mlpin,sae_mlpin, mlp, mlp_grads, ln.weight, cache["ln_pre_y"], cache["ln_scale"])
...
# sparse_block_args = (z_2_idx, z_idx, sae_mlpin, sae_mlpout, mlp, mlp_grads, ln.weight, cache["ln_pre_y"], cache["ln_scale"])
# sparse_block_times = benchmark_function(jacobian_full_block_sparse, sparse_block_args)
