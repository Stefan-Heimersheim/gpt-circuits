...
from xavier.utils import load_experiments_and_extract_data, prepare_plot_data, compute_auc_values
...
def main():
    ...
    data_dir = Path.cwd() / 'xavier' / 'experiments' / 'data' / 'jln_5P'
    method_labels = ["1e-6", "1e-3", "1e-0"]
    edge_sort = "gradient"
    layers = [2]
    ...
    # PLOT RESULTS
    plot_method_labels = ["1e-6", "1e-3", "1e-0"] # Adjusted labels for plotting
    ...
    for layer_idx in range(len(layers)):
        ...
        filename = Path.cwd() / 'xavier' / 'experiments' / 'plots' / f'FF_BLOCK_{layers[layer_idx]}.svg'
