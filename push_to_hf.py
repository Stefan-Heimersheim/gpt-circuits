...
def push_to_huggingface_with_name(directory, repo_name, dry_run=False):
    """
    Pushes all files from the specified directory to a Hugging Face model repo with a custom name.

    Args:
        directory: Path to the directory to upload
        repo_name: Custom name for the repository
        dry_run: If True, only print what would be uploaded without actually uploading
    """
    # Ensure the directory exists
    if not os.path.isdir(directory):
...
if __name__ == "__main__":
    ...
    # Create mutually exclusive group for directory modes
    ...
    group.add_argument(
        "--dir",
        type=str,
        help="Single directory to upload. Use with --name to specify the repo name."
    )
    
    parser.add_argument(
        "--name",
        type=str,
        help="Repository name to use (required when using --dir mode)"
    )
    parser.add_argument(
        "--dryrun",
        action="store_true",
    ...
    if args.dir:
        if not args.name:
            print("❌ Error: --name is required when using --dir mode")
            exit(1)
        ...
        print(f"🔍 Single directory mode: uploading '{args.dir}' to repo '{args.name}'")
        push_to_huggingface_with_name(args.dir, args.name, args.dryrun)
    
    # Handle pattern mode (existing functionality)
    else:
